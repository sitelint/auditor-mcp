...
function loadTranslations(): Record<string, { value: string }> {
...
function countBySeverity(entries: IViolationEntry[]): Array<{ severity: string; count: number }> {
...
export function formatAuditReport(result: IAuditResult): IFormattedReport {
  if (result.error) {
    return {
  ...
  const rules = report['rules'] as Record<string, Record<string, unknown>> | undefined;
  ...
  if (rules !== undefined) {
    for (const ruleKey of Object.keys(rules)) {
      ...
      const results = rule['results'] as Array<Record<string, unknown>> | undefined;

      if (statusType === 'error' && Array.isArray(results)) {
        const stdMeta = rule['standardMetaData'] as Record<string, unknown> | null | undefined;
        const stdConfig = stdMeta?.['config'] as Record<string, unknown> | null | undefined;
        const criterionNum = (stdConfig?.['num'] as string) ?? 'other';
        const criterionTitle = (stdConfig?.['title'] as string) ?? 'Non-WCAG Issues';
        const level = (stdConfig?.['level'] as string) ?? '';

        for (const resultItem of results) {
          const messages = resultItem['messageTranslation'] as Array<{
            messageTranslationId: string;
            messageTranslationParams?: string[] | null;
          }> | undefined;
          const element = resultItem['element'] as Record<string, unknown> | undefined;
          ...
          if (Array.isArray(messages)) {
            for (const msg of messages) {
              const resolved = resolveMessage(msg.messageTranslationId, msg.messageTranslationParams);
              messageTexts.push(resolved);
            }
          }

          const entry: IViolationEntry = { severity, ruleTitle, selector, messages: messageTexts };
  ...
  const sortedGroups = [...groups.values()].sort((a, b) => compareCriteria(a.criterionNum, b.criterionNum));

  // Sort entries within each group by severity weight
  for (const group of sortedGroups) {
    group.entries.sort((a, b) => (SEVERITY_WEIGHT[a.severity] ?? 99) - (SEVERITY_WEIGHT[b.severity] ?? 99));
  }

  const totalViolations = sortedGroups.reduce((sum, group) => sum + group.entries.length, 0);

  // Flatten entries respecting group order, then truncate
  const allEntries: Array<IViolationEntry & { criterionNum: string; criterionTitle: string; level: string }> = [];
  ...
  for (const group of sortedGroups) {
    ...
    for (let i = 0; i < takeFromGroup; i += 1) {
      ...
      allEntries.push({ ...entry, criterionNum: group.criterionNum, criterionTitle: group.criterionTitle, level: group.level });
    }
  }

  const shownEntries = allEntries;
  const truncatedCount = totalViolations - MAX_VIOLATIONS > 0 ? totalViolations - MAX_VIOLATIONS : 0;
  const truncated = truncatedCount > 0;

  const summaryLines: string[] = [];

  summaryLines.push(`Score: ${score}`);
  summaryLines.push(`Violations: ${totalViolations}, Passed rules: ${passedCount}, Skipped: ${skippedCount}`);
  ...
  if (totalViolations > 0) {
    ...
    for (const group of sortedGroups) {
      const levelStr = group.level !== '' ? ` [${group.level.toUpperCase()}]` : '';
      const sevBreakdown = countBySeverity(group.entries).map((s) => `${s.severity}: ${s.count}`).join(', ');

      summaryLines.push(`  ${group.criterionNum}${levelStr}: ${group.criterionTitle} — ${group.entries.length} violation(s)`);
    ...
    if (shownEntries.length > 0) {
      ...
      for (const entry of shownEntries) {
        ...
        summaryLines.push(`${idx}. [${entry.severity.toUpperCase()}] ${entry.criterionNum}${levelStr} — ${entry.ruleTitle}${selectorStr}`);
    ...
    if (truncated) {
      summaryLines.push('');
      summaryLines.push(`⚠ ${truncatedCount} more violations not shown. Use report ID to access full data.`);
    }
  }
